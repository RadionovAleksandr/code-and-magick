...
window.renderStatistics = function (ctx, names, times) {
  ...
  // Цикл на отрисовкустолбиков статистики
  ctx.rotate(-90 * Math.PI / 180);
  for (var i = 0; i <= names.length - 1; i++) {
    ctx.fillStyle = 'rgba(255, 0, 0, 1)';
    ctx.fillRect(-CLOUD_HEIGHT + 2 * GAP, CLOUD_X + GAP_BAR + (GAP_BAR + BAR_HEIGHT) * i, (BAR_WIDTH * times[i]) / maxTime, BAR_HEIGHT);
  }
  ...
  for (var i = 0; i <= names.length - 1; i++) {
    ctx.fillStyle = 'white';
    ctx.fillText(names[i], -CLOUD_HEIGHT + GAP, CLOUD_Y + GAP + FONT_GAP + (GAP + BAR_HEIGHT) * i);
  }
};
